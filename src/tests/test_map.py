...
from bamboo.map import Map, ProcessMap, ThreadMap, apply_map
from tests._fixtures.slides import SlideSvs


def test_map_basic():
    image = Brightfield(SlideSvs.path, level=2)
    results = Map(lambda x: x[-1].shape, Tiles(image, 128, 128))
...
def test_parallel_map_threads():
    image = Brightfield(SlideSvs.path, level=2)
    results_a = Map(lambda x: x[-1].shape, Tiles(image, 128, 128))
    results_b = ThreadMap(lambda x: x[-1].shape, Tiles(image, 128, 128), n_jobs=2)
...
def test_parallel_map_processes():
    image = Brightfield(SlideSvs.path, level=2)
    results_a = Map(lambda x: x[-1].shape, Tiles(image, 128, 128))
    results_b = ProcessMap(lambda x: x[-1].shape, Tiles(image, 128, 128), n_jobs=2)
    for a, b in zip(results_a, results_b):
        assert a == b
